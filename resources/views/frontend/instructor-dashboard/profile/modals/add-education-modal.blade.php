<div class="modal-header">
    <h5 class="modal-title" id="dynamic-modalLabel">{{ __('Ajouter une formation') }}</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
    <form action="{{ route('instructor.setting.education.store') }}" method="POST" class="instructor__profile-form">
        @csrf
        <div class="col-md-12">
            <div class="form-grp">
                <label for="company">{{ __('Organisation') }} <code>*</code></label>
                <input id="company" name="organization" type="text" value="" required>
            </div>
        </div>

        <div class="col-md-12">
            <div class="form-grp">
                <label for="position">{{ __('Diplôme') }} <code>*</code></label>
                <input id="position" name="degree" type="text" value="" required>
            </div>
        </div>
